...
import ShopBoardWriteUI from "./BoardComponent.presenter";
...
export default function ShopBoardWrite(
  ...
  const onClickSubmit = async (): Promise<void> => {
    ...
    if (
      ...
      success("글");
      void router.push(`/shopboards/${result.data?.createBoard._id}`);
    }
  };
  const onClickEdit = async (): Promise<void> => {
    ...
    try {
      ...
      success("수정");
      void router.push(`/shopboards/${result.data?.updateBoard._id}`);
    } catch (error) {
      Error();
    }
  };
